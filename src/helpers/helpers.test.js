...
describe('Ensure works of Encrypter', () => {
  test('Should retrun MissinParam if no params is provided', async () => {
    ...
    expect(async () => { await encrypter.compare() }).rejects.toThrow(new MissingParamError('value'))
    expect(async () => { await encrypter.compare(null, 'any_hash ') }).rejects.toThrow(new MissingParamError('value'))
  })
  test('Should retrun MissinParam if no params is provided', async () => {
    const encrypter = new Encrypter()
    expect(async () => { await encrypter.compare('any_value', null) }).rejects.toThrow(new MissingParamError('hash'))
  })
})
...
describe('Ensure works of TokenValidator', () => {
  ...
  test('Should retrun MissingParamError if id no has provided', async () => {
    const secret = 'any_secret'
    const token = new TokenHelper(secret)
    expect(async () => { await token.generate() }).rejects.toThrow(new MissingParamError('id'))
  })
  test('Should retrun JsonWebTokenError if has any error', async () => {
    const secret = 'any_secret'
    const accessToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJmb28iOiJiYXIiLCJpYXQiOjE0MzcwMTg1ODIsImV4cCI6MTQzNzAxODU5Mn0.3aR3vocmgRpG05rsI9MpR6z2T_BGtMQaPq2YR6QaroU'
